...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (payment.customer?.email && payment.customer.email.toLowerCase() !== String(token.email || '').toLowerCase()) {
      return send(res, 403, { error: 'This payment belongs to another account.' });
    }
    if (Number(payment.amount) !== 100000 || payment.currency !== 'NGN') {
    ...
    send(res, 200, { success: true, expiresAt: result.expiresAt, alreadyProcessed: result.alreadyProcessed });
  } catch (e) {
    send(res, e.statusCode || 500, { error: e.message || 'Unable to verify payment.' });
  }
};
